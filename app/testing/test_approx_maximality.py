...
#on this randomness are run 20 times each  
...
class MyTestCase(TestCase):
    ...
    def random_function_repeater(func):
        oneTrue = False
        oneFalse = False
        for _ in range(20):
            if func(): 
            ...
            if oneTrue and oneFalse:
                return True #Test passed, leave the function
        return False
           
    def test_outfix_approx(self):
        aut = openfile(NFAs[0])
        ...
        def singleOutfixTest():
            result = get_response(post, files, False)
            return result["result"].startswith("Yes")
        self.assertTrue(random_function_repeater(singleOutfixTest))

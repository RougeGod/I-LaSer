...
#     form = UploadFileForm()
#     response = {'form': form, decision ''}
#     return render_to_response('upload.html', response)
...
def getResponse(post, files, form = True):
    ...
    try:
        ...
        decision = "Please select a decision question."
    ...
    if question == "1" or question == "2":
        ...
        if property_type == "1":
            ...
            if 0 < int(fixed_type) < 6:
                ...
                pName = "code"
                proof = ''
                #prop = CodeProp()    # SHOULD CHANGE TO
        ...
        if limitTranP(aut.delta, prop.Aut.delta, LIMIT):
            decision = "Sizes of the automaton and transducer exceed limit! (See Technical Notes below)"
            return {'form':form, 'error_message': decision, 'automaton':autName, 'transducer':tName}
        else:
            proof = ""
            if question == "1":
        ...
        if (n_num == "" or s_num == "" or l_num == ""):
            decision = "Please input integers s,n,l."
            return {'form': form, 'error_message': decision}
        elif( 1 < int(s_num) > 10):
            decision = "S must be lesser than 10 and greater than 1"
            return {'form': form, 'error_message': decision}
        elif (int(s_num) > int(l_num)):
            decision = "S must be lesser than L"
            return {'form': form, 'error_message': decision}
        else:
            decision = ''
            witness = ''
            ...
            if property_type == "1":
                ...
                fixed_type = post['fixed_type']
                if 0 < int(fixed_type) < 6:
                    prop = createFixedProperty(alp, fixed_type)
                    try:
                ...
                if property_type == "2":
                    try:
                        prop = codes.IATProp(readOneFromString(tStr))
                    except IncorrectFormat:
                        decision = "The property file appears to be incorrectly formatted."
                        return {'form': form, 'error_message': decision, 'transducer': tName}
                    except TypeError:
...
def getCode(post, files, form=True, testMode=None):
    ...
    if testMode is not None:
        ...
        decision = "<a href='"+settings.MEDIA_URL+"%s.zip'> Download your code </a></br> (See 'Technical notes above')" % name

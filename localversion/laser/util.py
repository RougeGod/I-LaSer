...
from .FAdo.codes import IATProp, buildPrefixProperty, buildSuffixProperty, infixTransducer, \
                        buildOutfixProperty, buildHypercodeProperty, buildUDCodeProperty
...
def create_fixed_property(alphabet, fixed_type):
    ...
    if fixed_type in ["1", "PREFIX", 1]:
        result = buildPrefixProperty(alphabet)
    elif fixed_type in ["2", "SUFFIX", 2]:
        result = buildSuffixProperty(alphabet)
    elif fixed_type in ["3", "BIFIX", 3]:
        result = buildPrefixProperty(alphabet) & buildSuffixProperty(alphabet)
    elif fixed_type in ["4", "INFIX", 4]:
        result = IATProp(infixTransducer(alphabet))
    elif fixed_type in ["5", "OUTFIX", 5]:
        result = buildOutfixProperty(alphabet)
    elif fixed_type in ["6", "CODE", 6]:
        result = buildUDCodeProperty(alphabet)
    elif fixed_type in ["7", "HYPERCODE", 7]:
        result = buildHypercodeProperty(alphabet)
    return result
